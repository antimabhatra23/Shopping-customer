...
const Products = () => {
  ...
  const loadProducts = async (category = '', gender = '', color = '', price = '', size = '', rating = '') => {
    try {
      setLoading(true);
      const response = await axios.get(`https://shopping-backend-beryl.vercel.app/products?category=${category}&gender=${gender}&color=${color}&price=${price}&size=${size}&rating=${rating}`);
